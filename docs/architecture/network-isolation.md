...
- To keep the sidecar's DNS filtering and Credential Vault while a CNI policy does the
  address-level enforcement, run it with `OPENSANDBOX_EGRESS_ENFORCEMENT=external`. The
  sidecar then installs no netfilter rules and needs no `CAP_NET_ADMIN`, so it starts
  under gVisor; in exchange the deployment must point the sandbox's resolver and
  `HTTPS_PROXY` at it, because nothing redirects traffic into it any more. See
  [Enforcement outside the pod](../components/egress.md#enforcement-outside-the-pod).
