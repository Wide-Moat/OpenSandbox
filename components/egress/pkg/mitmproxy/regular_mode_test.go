...
//
// ⚠ WITH ListenV6 SET, as it is in production: the launcher leaves it nil and Launch
// fills it from loopbackV6Available(), true in any pod with ::1. Left nil here, the
// [::1] transparent listener cannot be emitted at all, and the check below that none
// sits beside the regular one could not fail.
func argsForEnv(port int) []string {
	v6 := true
	return buildMitmdumpArgs(configFromEnvForTest(Config{ListenPort: port, ListenV6: &v6}))
}
...
func TestWM1RegularModeIsSetWhenEnforcementIsExternal(t *testing.T) {
	...
	require.True(t, strings.HasPrefix(args[mode+1], "regular@"),
		"--mode must name a regular@host:port spec, as its own argv entry; got %q", args[mode+1])
	for i, a := range args {
		if a == "--mode" && i+1 < len(args) {
			require.False(t, strings.HasPrefix(args[i+1], "transparent"),
				"a transparent listener beside the regular one waits for redirected "+
					"connections that never arrive under external enforcement")
		}
	}
...
// The guard: without external enforcement mitmdump must be told exactly what upstream
// tells it -- a transparent listener on each loopback family the redirect delivers to --
// or every existing deployment's transparent interception becomes a proxy nobody is
// pointed at. Asserted on the full list of --mode values, so a regular spec slipped in
// beside the transparent ones fails here too.
func TestWM1TransparentStaysTheDefault(t *testing.T) {
	t.Setenv("OPENSANDBOX_EGRESS_ENFORCEMENT", "")

	v6 := true
	args := buildMitmdumpArgs(configFromEnvForTest(Config{ListenPort: 18081, ListenV6: &v6}))
	var modes []string
	for i, a := range args {
		if a == "--mode" && i+1 < len(args) {
			modes = append(modes, args[i+1])
		}
	}
	require.Equal(t, []string{"transparent@127.0.0.1:18081", "transparent@::1:18081"}, modes,
		"without external enforcement mitmdump must get upstream's transparent specs, unchanged")
	require.NotContains(t, strings.Join(args, " "), "mode=",
		"the mode travels as --mode; --set does not replace a sequence option")
}
