...
const (
	...
	// outside it — a CNI policy on the host side of the veth, an admission policy —
	// already constrains the traffic, and the sidecar must not try to install
	// netfilter rules of its own. That is not a preference: under a sandboxed kernel
	// such as gVisor, nftables and iptables do not exist as subsystems and
	// CAP_NET_ADMIN cannot be granted, so the redirect setup fails at startup and the
	// component crash-loops. The credential proxy still works in this mode, because
	// clients reach mitmproxy as an explicit proxy rather than by being redirected
	// into it. The DNS filter does not: it listens on a port no resolver setting can
	// name, so without the redirect nothing in the pod enforces the egress policy.
