...
// The flag, parsed as main parses it, reaches the manager's cache. The tests above call
// applyWatchNamespaces directly and TestWM6TheFlagIsOffered only reads -h, so neither
// notices the value being accepted and then never applied -- a cluster-wide cache
// behind a flag that says otherwise.
func TestWM6TheFlagReachesTheManagerCache(t *testing.T) {
	fs := flag.NewFlagSet("controller", flag.ContinueOnError)
	scopeCache := registerWatchNamespacesFlag(fs)
	if err := fs.Parse([]string{"--watch-namespaces=sandbox-prod,sandbox-stage"}); err != nil {
		t.Fatal(err)
	}

	options := scopeCache(ctrl.Options{})

	got := options.Cache.DefaultNamespaces
	if len(got) != 2 {
		t.Fatalf("expected the two namespaces from the flag, got %v", got)
	}
	for _, want := range []string{"sandbox-prod", "sandbox-stage"} {
		if _, ok := got[want]; !ok {
			t.Fatalf("expected %s among the watched namespaces, got %v", want, got)
