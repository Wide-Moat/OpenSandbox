...
| `enforcement` | string | `"sidecar"` | Where egress is enforced, passed to the sidecar as `OPENSANDBOX_EGRESS_ENFORCEMENT`. **`sidecar`** — the sidecar installs the DNS and HTTP redirects in the sandbox netns and gets `NET_ADMIN`; **`external`** — something outside the pod (a CNI-level policy) constrains the traffic, so the sidecar installs no netfilter rules, gets no capabilities, and **nothing in the pod enforces a sandbox's `networkPolicy`** any more. Kubernetes only: the Docker runtime never passes it to its sidecar. `external` requires `mode = "dns"` and `disable_ipv6 = false` and rules out `upstream_proxy`; each pairing is refused at load. See [Enforcement outside the pod](../docs/architecture/network/egress.md#enforcement-outside-the-pod). |
